...
# this is unrelated to twp but handles toolpath in the preview

h.newpin("kinstype-select", hal.HAL_FLOAT, hal.HAL_IN)
h.newpin("tooltip-rotate-x", hal.HAL_FLOAT, hal.HAL_OUT)
h.newpin("tooltip-rotate-y", hal.HAL_FLOAT, hal.HAL_OUT)
h.newpin("tooltip-rotate-z", hal.HAL_FLOAT, hal.HAL_OUT)
...
try:
    while 1:
        ...
        # handle the switch in location of the tooltip capture in the gui
        if h['kinstype-select'] in [0,1,3]:
            h['tooltip-rotate-x'] = 180
            h['tooltip-rotate-y'] = 90
            h['tooltip-rotate-z'] = -90
        else:
            h['tooltip-rotate-x'] = 90
            h['tooltip-rotate-y'] = 0
            h['tooltip-rotate-z'] = 0
