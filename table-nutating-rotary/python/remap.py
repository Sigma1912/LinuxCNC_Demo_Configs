...
kins_post_rotation = kins_comp + '.post-rot'
...
try:
    ...
    log.error("Unable to parse orientation priority given in INI .")
...
def kins_calc_secondary(self, tool_z_req, tool_x_req, theta_1_list):
    global joint_letter_primary, joint_letter_secondary
...
# Note: this uses functions derived from the custom kinematic
def kins_calc_primary(self, tool_z_req):
    global joint_letter_primary, joint_letter_secondary
    ...
    theta_1_list=[]
    (Kzx, Kzy, Kzz) = (tool_z_req[0], tool_z_req[1], tool_z_req[2])
    log.debug('Calculating possible primary angles.')
    ...
    if (joint_letter_primary, joint_letter_secondary)== ('B', 'C'):
        ...
        if Kzz < 2*Cv*Cv - 1:
            log.error('Requested orientation not reachable with the current nutation angle!')
        else:
            u = radians(hal.get_value(kins_table_angle))
            Su = sin(u)
            Cu = cos(u)
            ...
            for theta in [theta_1, -theta_1]:
                log.debug('Checking if result %s is within primary joint limits of %s and %s.',
                            degrees(theta), primary_min_limit, primary_max_limit)
                if degrees(theta) > primary_min_limit and degrees(theta) < primary_max_limit:
                    log.debug('Adding %s to valid angles list.', degrees(theta))
                    theta_1_list.append(theta)
            log.debug('List of possible primary angles: %s,\n', theta_1_list)
    else:
        log.error('No formula for this spindle kinematic (primary, secondary) %s', (joint_letter_primary, joint_letter_secondary))
    return theta_1_list
...
def kins_calc_jnt_angles(self, tool_z_req, tool_x_req):
    ...
    theta_1_calcd = kins_calc_primary(self, tool_z_req)
    # calculate secondary joint values using kinematic specific formula
    theta_2_calcd = kins_calc_secondary(self, tool_z_req, tool_x_req, theta_1_calcd)
    angle_pairs_list = []
    ...
    for i in range(len(angle_pairs_list)):
        ...
        t_out = kins_tool_transformation(angle_pairs_list[i][0], angle_pairs_list[i][1], 0, matrix_in,'fwd')
...
# calculates the required post-rotation around tool-z so the tool-x matches the requested
# orientation after rotation of the spindle joints
def kins_calc_post_rot(self, theta_1, theta_2, tool_x_req, matrix_in):
    ...
    # run the twp matrix through the tool kinematic transformation in the requested direction
    # using the given joint angles and post-rotation zero
    ...
    # we calculate the angular difference between the two vectors so we can 'post-rotate'
    ...
    if np.dot(tool_x_is,tool_x_req) >  1 - epsilon:
        log.info("Tool x-vector already oriented, setting post-rotation = 0")
        # if we are already parallel then we don't need to post-rotate
        post_rot = 0
    else:
        ...
        post_rot =  np.arccos(np.dot(tool_x_req, tool_x_is))
        log.debug('base post_rot: %s', post_rot)
        # To find out which quadrant we need the angle to be in we create a list of them all
        post_rot_list = [post_rot, -post_rot, 2*pi-post_rot, -(2*pi-post_rot)]
        log.debug('post_rot_list: %s',post_rot_list)
        # then we run all of them through the kinematic model and see which gives us
        # the requested tool-x-vector
        for post_rot in post_rot_list:
            zeta = 0.1
            # run the identity matrix through the tool kinematic transformation in the requested direction
            # using the given joint angles and post-rotation angle in the list
            t_out = kins_tool_transformation(theta_1, theta_2, post_rot, matrix_in,'inv')
            ...
            if match:
                ...
                break
        log.info("post-rotation calculated [deg]: %s\n", degrees(post_rot))
    # return post_rot in radians
    return post_rot
...
def kins_calc_tool_transformation(self, matrix_in, theta_1=None, theta_2=None, post_rot=None, direction='fwd'):
    global kins_post_rotation
    ...
    # post-rot is the virtual rotary axis around the tool-z axis to align the tool-x axis
    # if no post-rot angle is passed then we use the currently active value
    if post_rot == None:

        post_rot =  hal.get_value(kins_post_rotation )
        log.debug("current post-rot: %s", post_rot)
    else:
        log.debug("requested post-rot value [DEG]): %s", degrees(post_rot))
    # run the input matrix through the tool kinematic transformation in the requested direction
    # using the current joint angles and post-rotation as requested
    matrix_out = kins_tool_transformation(theta_1, theta_2, post_rot, matrix_in, direction)
    return matrix_out
...
def g53x_core(self):
    ...
    orient_mode = p
    # calculate the required rotary joint positions and post_rotation for the requested tool-orientation
    try:
    ...
    post_rot = kins_calc_post_rot(self,theta_1, theta_2, (1,0,0), twp_matrix)
    ...
    kins_rot_point_abs = (hal.get_value(kins_rot_point_abs_x), hal.get_value(kins_rot_point_abs_y), hal.get_value(kins_rot_point_abs_z))
    log.debug("G53.x: Got sbsolute position of rotation point: %s", kins_rot_point_abs)
    work_offset_from_cntr_rot_table = tuple(np.subtract(saved_work_offset, kins_rot_point_abs))
    log.debug("G53.x: Work offset from center of rotary table: %s", work_offset_from_cntr_rot_table)
...
# tool-orientation
def g683(self, **words):
    global twp_matrix, post_rot, twp_flag, saved_work_offset_number, saved_work_offset
   ...
   ###post_rot = kins_calc_tool_rot_c_for_horizontal_x(self, theta_1, theta_2 )
    ...
    # transformation using the current rotary joint positions and calculated post-rotation angle
    ...
    twp_matrix = kins_calc_tool_transformation(self, start_matrix, None, None, post_rot +  radians(r), 'fwd')
...
def g682(self, **words):
    global twp_matrix, post_rot, twp_flag, twp_build_params, saved_work_offset_number, saved_work_offset
    ...
    if p == 0: # true euler angles (this is the default mode)
        ...
        twp_origin_rotation = Rz(radians(r))
