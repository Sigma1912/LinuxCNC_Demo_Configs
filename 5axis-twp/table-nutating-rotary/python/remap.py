...
try:
    ...
    log.error("Unable to parse orientation priority given in INI. Setting it to 1.")
...
def kins_calc_jnt_angles(tool_z_req, tool_x_req):
    print('-------------kins_calc_jnt_angles')
    log.debug('tool_z_requested: %s', tool_z_req)
    log.debug('tool_x_requested: %s', tool_x_req)
    ...
    try:
        (theta_1_calcd, theta_2_calcd) = kins_calc_possible_joint_angles(tool_z_req, tool_x_req)
    except Exception as error:
    ...
    for i in range(len(angle_pairs_list)):
        ...
        try:
            direction = kins_calc_tool_transformation_get_direction()
        except Exception as error:
            log.error('kins_calc_tool_transformation_get_direction, %s', error)
        try:
            t_out = kins_tool_transformation(angle_pairs_list[i][0], angle_pairs_list[i][1], 0, matrix_in, direction)
        except Exception as error:
            log.error('kins_calc_jnt_angles, %s', error)
        ...
        log.debug('tool_z_diff: %s', tool_z_diff)
        # and check if all elements are within [-epsilon,epsilon]
        match_z = np.all((tool_z_diff > -epsilon) & (tool_z_diff < epsilon))
        log.debug('Is the tool-Z-vector close enough ? %s', match_z)
        if match_z:
...
# calculates the required virtual-rotation around tool-z so the tool-x matches the requested
# orientation after rotation of the spindle joints
def kins_calc_virtual_rot(theta_1, theta_2, tool_x_req, tool_z_req=None, matrix_in=None):
    ...
    # run matrix_in through the tool kinematic transformation in the requested direction
    # using the given joint angles and virtual-rotation zero
    ...
    # we calculate the angular difference between the two vectors so we can 'virtual-rotate'
    ...
    if np.dot(tool_x_is,tool_x_req) >  1 - epsilon:
        log.info("Tool x-vector already oriented, setting virtual-rotation = 0")
        # if we are already parallel then we don't need to virtual-rotate
        virtual_rot = 0
    else:
        ...
        virtual_rot =  np.arccos(np.dot(tool_x_req, tool_x_is))
        ...
        for virtual_rot in virtual_rot_list:
            ...
            try:
                t_out = kins_tool_transformation(theta_1, theta_2, virtual_rot, matrix_in,'inv')
            except Exception as error:
                log.error('kins_calc_virtual_rot, %s', error)
...
def kins_calc_tool_transformation(self, matrix_in, theta_1=None, theta_2=None, virtual_rot=None, direction='fwd'):
    ...
    # virtual-rot is the virtual rotary axis around the tool-z axis to align the tool-x axis
    # if no virtual-rot angle is passed then we use the currently active value
    if virtual_rot == None:
        virtual_rot =  hal.get_value(kins_virtual_rotation )
        log.debug("current virtual-rot: %s", virtual_rot)
    else:
    ...
    try:
        matrix_out = kins_tool_transformation(theta_1, theta_2, virtual_rot, matrix_in, direction)
    except Exception as error:
        log.error('kins_calc_tool_transformation, %s', error)
...
def g53x_core(self):
    ...
    orient_mode = p
    # calculate the required rotary joint positions and virtual_rotation for the requested tool-orientation
    try:
        ...
        possible_prim_sec_angle_pairs = kins_calc_jnt_angles(tool_z_requested, tool_x_requested)
    ...
    if len(possible_prim_sec_angle_pairs) == 0 :
        # reset the twp parameters
        log.error('G53.x: No possible primary/secondary angle pairs found., %s', error)
        reset_twp_params()
        msg = "G53.x ERROR: Requested tool orientation not reachable -> aborting G53.x"
        log.debug(msg)
        emccanon.CANON_ERROR(msg)
    ...
    # calculate the virtual-rotation needed so our tool-x vector matches the requested tool-x vector
    try:
        (tool_x_requested, tool_z_requested, matrix_in) = kins_calc_virtual_rot_get_values(tool_x_requested, tool_z_requested, twp_matrix)
    except Exception as error:
        log.error('G53.x: kins_calc_virtual_rot_get_values failed, %s', error)

    virtual_rot = kins_calc_virtual_rot(\
                       radians(theta_1), radians(theta_2), tool_x_requested, tool_z_requested, matrix_in)
    log.debug("Calculated virtual-rotation (virtual_rot) to match requested tool-x): %s", virtual_rot)

    # mark twp-flag as active
    twp_flag = [0, 'active']
    gui_update_twp()
    # set the virtual-rotation value in the kinematic component
    log.debug("G53.x: setting primary, secondary and virtual_rotation angles in kinematic component: %s",\
              (theta_1, theta_2, degrees(virtual_rot)))
    log.debug("G53.x: Setting angle values in kins comp to (theta_1, theta_2, virtual_rot): %s", (theta_1, theta_2, virtual_rot))
    try:
        kins_set_values(theta_1, theta_2, virtual_rot)
    except Exception as error:
        log.error('G53.x: kins_set_values failed, %s', error)
    ...
    try:
        ...
        log.error('G53.x: Calculation of kins_calc_transformed_work_offset failed, %s', error)
    log.debug("G53.x:new_offset: %s", new_offset)

    log.debug("G53.x: Setting transformed work-offsets for tool-kins in G59, G59.1, G59.2 and G59.3 to: %s ", new_offset)
...
def g683(self, **words):
    ...
    theta_1, theta_2 = get_current_rotary_positions(self)
    # calculate tool-virtualrotation necessary to have tool-x vector in machine xy-plane
    try:
        virtual_rot = kins_calc_tool_rot_c_for_horizontal_x(theta_1, theta_2 )
    except Exception as error:
        log.error('remap_func: kins_calc_tool_rot_c_for_horizontal_x failed, %s', error)
    ...
    # calculated virtual-rotation to get the tool-x vector in the machine xy-plane
    # for this we take the 4x4 identity matrix and pass it through the inverse tool kinematic
    # transformation using the current rotary joint positions and calculated virtual-rotation angle
    ...
    try:
        ...
        log.error('kins_calc_tool_transformation_get_direction, %s', error)
    twp_matrix = kins_calc_tool_transformation(start_matrix, None, None, virtual_rot +  radians(r), direction)
