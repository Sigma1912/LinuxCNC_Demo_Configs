...
class HAL_Multlabel(Gtk.Label, _HalWidgetBase):
    __gtype_name__ = "HAL_Multilabel"
    __gsignals__ = dict([hal_pin_changed_signal])
    __gproperties__ = {
        'label_pin_type'  : ( GObject.TYPE_INT, 'HAL pin type', '0:S32 1:Float 2:U32',
                0, 2, 0, GObject.ParamFlags.READWRITE|GObject.ParamFlags.CONSTRUCT),
        'legend0' : ( GObject.TYPE_STRING, 'legend0',
                'Text to display',
                "%s", GObject.ParamFlags.READWRITE|GObject.ParamFlags.CONSTRUCT),
        'legend1' : ( GObject.TYPE_STRING, 'legend1',
                'Text to display',
                "%s", GObject.ParamFlags.READWRITE|GObject.ParamFlags.CONSTRUCT),
        'legend2' : ( GObject.TYPE_STRING, 'legend2',
                'Text to display',
                "%s", GObject.ParamFlags.READWRITE|GObject.ParamFlags.CONSTRUCT),
        'legend3' : ( GObject.TYPE_STRING, 'legend3',
                'Text to display',
                "%s", GObject.ParamFlags.READWRITE|GObject.ParamFlags.CONSTRUCT),
    }
    ...
    def do_get_property(self, property):
        name = property.name.replace('-', '_')
        if name in ['label_pin_type', 'legend0', 'legend1', 'legend2', 'legend3']:
            return getattr(self, name)
        else:
            raise AttributeError('unknown property %s' % property.name)
    ...
    def _hal_init(self):
        self.hal_pin_0 = self.hal.newpin(self.hal_name + ".legend0", hal.HAL_BIT, hal.HAL_IN)
        self.hal_pin_1 = self.hal.newpin(self.hal_name + ".legend1", hal.HAL_BIT, hal.HAL_IN)
        self.hal_pin_2 = self.hal.newpin(self.hal_name + ".legend2", hal.HAL_BIT, hal.HAL_IN)
        self.hal_pin_3 = self.hal.newpin(self.hal_name + ".legend3", hal.HAL_BIT, hal.HAL_IN)

#        self.hal_pin.connect('value-changed', lambda p: self.set_text(self.legend0))
#        self.hal_pin.connect('value-changed', lambda s: self.emit('hal-pin-changed', s))


    def hal_update(self, *a):
        if self.hal_pin_0.get() == 1:
           self.set_text(self.legend0)
           self.modify_bg(Gtk.StateType.NORMAL, Gdk.color_parse("dark sea green"))
           self.modify_fg(Gtk.StateType.NORMAL, Gdk.color_parse("black"))
        elif self.hal_pin_1.get() == 1:
           self.set_text(self.legend1)
           self.modify_bg(Gtk.StateType.NORMAL, Gdk.color_parse("light salmon"))
           self.modify_fg(Gtk.StateType.NORMAL, Gdk.color_parse("black"))
        elif self.hal_pin_2.get() == 1:
           self.set_text(self.legend2)
           self.modify_bg(Gtk.StateType.NORMAL, Gdk.color_parse("light pink"))
           self.modify_fg(Gtk.StateType.NORMAL, Gdk.color_parse("black"))
        elif self.hal_pin_1.get() == 1:
           self.set_text(self.legend3)
